# Code File for calcluating descriptive statistics of CPSC
...
KEYSUM_E <- KEYFIRM[, list(wastewater=sum(wastewater_e, na.rm = TRUE), cod=sum(cod_e, na.rm = TRUE), nh=sum(nh_e, na.rm = TRUE), pet=sum(pet_e, na.rm = TRUE), phe=sum(phe_e, na.rm = TRUE), bod=sum(bod_e, na.rm = TRUE), cyn=sum(cyn_e, na.rm = TRUE), as=sum(as_e, na.rm = TRUE), chr=sum(chr_e, na.rm = TRUE), chr6=sum(chr6_e, na.rm = TRUE)), by=list(industry_a)]

# Calculate the percentage
KEYPER_E <- KEYSUM_E[, list(wastewater = 100*wastewater/sum(wastewater), cod = 100*cod/sum(cod), nh = 100*nh/sum(nh), pet = 100*pet/sum(pet), phe = 100*phe/sum(phe), bod = 100*bod/sum(bod), cyn = 100*cyn/sum(cyn), as = 100*as/sum(as), chr = 100*chr/sum(chr), chr6 = 100*chr6/sum(chr6), industry_a = industry_a)]
...
ALLSUM_E <- ALLFIRM[, list(wastewater=sum(wastewater_e, na.rm = TRUE), cod=sum(cod_e, na.rm = TRUE), nh=sum(nh_e, na.rm = TRUE), pet=sum(pet_e, na.rm = TRUE), phe=sum(phe_e, na.rm = TRUE), bod=sum(bod_e, na.rm = TRUE), cyn=sum(cyn_e, na.rm = TRUE), as=sum(as_e, na.rm = TRUE)), by=list(industry_a)]

## Compare the relative contribution of key and regular firms
ALLCOMP_E <- ALLFIRM[, list(wastewater=sum(wastewater_e, na.rm = TRUE), cod=sum(cod_e, na.rm = TRUE), nh=sum(nh_e, na.rm = TRUE), pet=sum(pet_e, na.rm = TRUE), phe=sum(phe_e, na.rm = TRUE), bod=sum(bod_e, na.rm = TRUE), cyn=sum(cyn_e, na.rm = TRUE), as=sum(as_e, na.rm = TRUE), product = sum(product, na.rm = TRUE)), by=list(industry_a, Census_Type)]
AC_PRES <- ALLCOMP_E
# Calculate the relative percentage
ALLAUX_E <- ALLFIRM[, list(wastewater=sum(wastewater_e, na.rm = TRUE), cod=sum(cod_e, na.rm = TRUE), nh=sum(nh_e, na.rm = TRUE), pet=sum(pet_e, na.rm = TRUE), phe=sum(phe_e, na.rm = TRUE), bod=sum(bod_e, na.rm = TRUE), cyn=sum(cyn_e, na.rm = TRUE), as=sum(as_e, na.rm = TRUE), product = sum(product, na.rm = TRUE)), by=list(industry_a)]
...
for (i in foo1){
  ALLCOMP_E$wastewater[i] <- 100*ALLCOMP_E$wastewater[i]/ALLAUX_E$wastewater[(i+1)/2]
  ...
  ALLCOMP_E$product[i] <- 100*ALLCOMP_E$product[i]/ALLAUX_E$product[(i+1)/2]
