...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item
        There are only two in-text numbers that require manual calculation: the average reduction in intensity for polluting industries and for the whole manufacturing sector (Section \rom{2} point 5 of the README). They are now automated in \bcode{Accounting.R} as well. The code and instructions are both updated accordingly.
    \end{itemize}
    \item
    MATLAB Terminal:
    \begin{itemize}
        \item
        The programs and instructions for the Online Appendix had already been uploaded in our first submission.
        \item
        We suspect that the reason that the data editor did not see the computation accuracy related information (optimality tolerance, etc.) is due to a different version of MATLAB being used. In the code, we have specified the option \bcode{'final-detailed'} when calling \bcode{fsolve} to request MATLAB to print information regarding computation accuracy. Such information was correctly printed when we tested our code in MATLAB R2014a, R2016a and R2017b. We believe that minor revisions to the syntax should be sufficient for other versions.
    \end{itemize}
\end{enumerate}
